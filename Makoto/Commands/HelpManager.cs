...
namespace Makoto.Commands
{
    public class HelpManager : ModuleBase<SocketCommandContext>
    {
        ...
        [Command("help"), Summary("More information on a specified command")]
        public async Task HelpAsync([Remainder, Summary("Command")]string command)
        {
            ...
            foreach (var match in result.Commands)
            {
                ...
                builder.AddField(x =>
                {
                    ...
                    x.Value = $"Parameters: {string.Join(", ", cmd.Parameters.Select(p => p.Summary))}\n" +
                                $"Summary: {cmd.Summary}";
                    x.IsInline = false;
                });
            }
